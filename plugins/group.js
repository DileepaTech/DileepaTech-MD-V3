...
bot(
	...
	async (message, match) => {
		...
		if (!isImAdmin) return await message.send(`_I'm not admin Promote DileepaTech Md-V3._`)
		...
		if (isUserAdmin) return await message.send(`_User is admin You Wont Kick._`)
		if (match == 'all') {
			user = participants
