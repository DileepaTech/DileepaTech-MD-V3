...
bot(
	...
	async (message, match) => {
		...
		await message.send('_Blocked By DileepaTech Md-V3_')
...
bot(
	...
	async (message, match) => {
		const id =
			...
			(!message.isGroup && message.jid)
		if (!id) return await message.send('*Give me a person*')
		await message.send('_Unblocked By DileepaTech Md-V3_')
...
bot(
	...
	async (message, match) => {
		...
		return await message.send('_Profile Picture Updated By DileepaTech Md-V3_')
	}
)
